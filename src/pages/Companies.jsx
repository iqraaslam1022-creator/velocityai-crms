...
import { Plus, Search, Building2, MoreHorizontal, Globe } from 'lucide-react';
import { DropdownMenu, DropdownMenuContent, DropdownMenuItem, DropdownMenuTrigger } from '@/components/ui/dropdown-menu';
import { useToast } from '@/components/ui/use-toast';

const defaultForm = { name: '', industry: '', website: '', phone: '', address: '', notes: '' };
...
export default function Companies() {
  ...
  useEffect(() => { if (organization?.id) loadCompanies(); }, [organization?.id]);

  const loadCompanies = async () => {
    setLoading(true);
    const data = await base44.entities.Company.filter({ org_id: organization.id }, '-created_date', 200);
    setCompanies(data);
    setLoading(false);
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editId) {
        await base44.entities.Company.update(editId, data);
      } else {
      ...
      toast({ title: 'Failed to save company', description: e.message, variant: 'destructive' });
  ...
  const filtered = companies.filter(c => !search || `${c.name} ${c.industry}`.toLowerCase().includes(search.toLowerCase()));
  ...
  return (
    <div className="space-y-5">
      ...
      {filtered.length === 0 ? (
        <EmptyState icon={Building2} title="No companies found" description={search ? 'Try a different search' : 'Add your first company to get started'} actionLabel={!search ? 'Add Company' : undefined} onAction={!search ? () => setShowForm(true) : undefined} />
      ) : (
              ...
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 capitalize">{c.industry || 'No industry set'}</p>
                  </div>
                </div>
                <DropdownMenu>
      ...
      <Dialog open={showForm} onOpenChange={setShowForm}>
        <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
          ...
          <div className="space-y-4 mt-2">
            <div><Label className="text-xs">Company Name *</Label><Input value={form.name} onChange={e => setForm({...form, name: e.target.value})} className="mt-1" /></div>
            ...
            <div className="flex gap-2 justify-end">
              ...
              <Button onClick={handleSave} disabled={saving} className="bg-indigo-600 hover:bg-indigo-700">
                {saving ? 'Saving...' : editId ? 'Update' : 'Create Company'}
              </Button>
            </div>
          </div>
        </DialogContent>
